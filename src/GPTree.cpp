...
struct Graph //无向图结构
{
	...
	vector<int> Split_Naive(Graph &G1, Graph &G2) //将子图一分为二返回color数组，并将两部分分别存至G1，G2 Kernighan-Lin algorithm
	{
		...
		ans /= 2;
		if (DEBUG1)
			printf("Split_Build_Heap over\n");
		//for(i=0;i<n;i++)printf("%d %d %d\n",i,color[i],con[i]);
		if (DEBUG1)
			cout << "start_ans=" << ans << endl;
	...
	struct cmp
	{
		bool operator()(const state &a, const state &b) { return a.len > b.len; }
...
struct G_Tree
{
	...
	struct Node
	{
		...
		void write()
		{
			...
			order.write();
			printf("borders:");
			for (map<int, pair<int, int> >::iterator iter = borders.begin(); iter != borders.end(); iter++)
			...
			printf("\n");
			printf("catch_dist ");
			for (int i = 0; i < catch_dist.size(); i++)
				printf("(%d,%d)", i, catch_dist[i]);
			printf("\n");
			printf("min_car_dist ");
			for (int i = 0; i < min_car_dist.size(); i++)
				printf("(i:%d,D:%d,id:%d)", i, min_car_dist[i].first, min_car_dist[i].second);
			printf("\n");
		}
	};
	...
	void push_borders_up_catch(int x, int bound = INF) //将S到结点x边界点缓存在x.catch_dist的最短路长度，计算S到x.father真实border的距离更新x.father.catch
	{
		if (node[x].father == 0)
			return;
		int y = node[x].father;
		if (node[x].catch_id == node[y].catch_id && bound <= node[y].catch_bound)
			return;
		...
		for (int i = 0; i < node[x].borders.size(); i++)
			if (node[x].border_in_father[i] != -1)
			{
				if (node[x].catch_dist[i] < bound) //bound界内的begin
					...
					(*dist2)[node[x].border_in_father[i]] = -1; //bound界外的begin
			}
		int **dist = node[y].dist.a;
		...
		for (int i = 0; i < node[x].borders.size(); i++)
			if (node[x].son[node[x].color[node[x].border_id_innode[i]]] == y)
			{
				if (node[x].catch_dist[i] < bound) //bound界内的begin
					...
					(*dist2)[node[x].border_in_son[i]] = -1; //bound界外的begin
			}
		int **dist = node[y].dist.a;
		int *begin, *end; //已算出的序列编号,未算出的序列编号
	...
	vector<int> KNN(int S, int K, vector<int> T) //计算S到T数组中的前K小并返回其在T数组中的下标
	{
		...
		for (int i = 0; i < T.size(); i++)
			query.push_back(make_pair(-node[find_LCA(id_in_node[S], id_in_node[T[i]])].deep, i));
	...
	vector<int> KNN(int S, int K, vector<int> T, vector<int> offset) //计算S到T数组中的前K小并返回其在T数组中的下标,考虑车到结点距离offset
	{
		...
		for (int i = 0; i < T.size(); i++)
		{
			...
			//if(bound==INF)printf("- ");else printf("%d ",bound);
			if (Optimization_KNN_Cut)
				ans.push_back(search_catch(S, T[query[i].second], bound) + offset[query[i].second]);
			else
				ans.push_back(search_catch(S, T[query[i].second]) + offset[query[i].second]);
			if (K_Value.size() < K)
				K_Value.push(ans[i]);
			else if (ans[i] < K_Value.top())
			{
	...
	vector<int> Range(int S, int R, vector<int> T, vector<int> offset) //计算S到T数组中距离小于R的终点T并返回其在T数组中的下标,考虑车到结点距离offset
	{
		...
		for (int i = 0; i < T.size(); i++)
		{
			if (offset[i] + search_catch(S, T[i], Optimization_KNN_Cut ? R : INF) < R)
				re.push_back(i);
		}
		return re;
	}
	...
	{
		if (DEBUG_)
			printf("push_borders_up_del_min_car_dist x=%d id=%d \n", x, start_id);
		int re = false;
		if (node[x].father == 0)
			return false;
		int y = node[x].father;
  ...
  public:
	...
	int request(pair<int, int> OD, vector<int> car_set) //规划新的OD请求应归于哪辆集合car_set中的车比较合适，并将其规划如车的路线中，并返回车的ID
	{
		...
		//欧几里得规划裁剪
		{
			...
			for (int i = 0; i < n; i++)
				cars[car_set[i]].push(OD.first, 1);
			for (int i = 0; i < n; i++)
				cars[car_set[i]].push(OD.second, 1);
			for (int i = 0; i < n; i++)
			{
				vector<int> order;
		...
		//真实规划
		for (int i = 0; i < n; i++)
			cars[car_set[i]].push(OD.first);
		for (int i = 0; i < n; i++)
			cars[car_set[i]].push(OD.second);
		for (int i = 0; i < n; i++)
		{
			vector<int> order;
		...
		for (int i = 0; i < n; i++)
			if (i != best_car_id)
			{
				...
				cars[car_set[i]].solve_value(cars[car_set[i]].LastOrderList);
		...
		for (int i = 0; i < car.ODlist.size(); i++)
		{
			if (car.ODlist[i] == node_id)
			{
				if (i & 1)
				{
					car.pop(i);
					car.pop(i - 1);
					car.solve_value(car.LastOrderList);
				}
				else
					car.ODlist[i] = -1;
			}
	...
	struct vehicle
	{
		...
		void push(int id, int type = 0) //向ODlist后加入一个新点id并重做dist;type==0表示使用路网距离，type=1表示使用欧几里得距离Euclidean_Distance
		{
			...
			for (int i = 0; i < re.n; i++)
				if (type == 0)
					re.a[i][re.n - 1] = re.a[re.n - 1][i] = tree.search_catch(id, ODlist[i]);
				else
					re.a[i][re.n - 1] = re.a[re.n - 1][i] = Euclidean_Distance(id, ODlist[i]);
			if (type == 0)
				pos_to_ODlist.push_back(tree.search_catch(id, position));
			else
				pos_to_ODlist.push_back(Euclidean_Distance(id, position));
			dist = re;
		}
...
int search(int S, int T) {return tree.search(S, T) ;}
int search_catch(int S,int T) {return tree.search_catch(S, T) ;};
